...
for(const protocol of ['uci','ucci']){
 ...
 let pending='',diagnostic='';const lines=[],moves=[],positions=[];
 ...
 try{
  ...
  assert.equal(moves.length,24,'bounded game should reach 24 plies');
  ...
  games.push({protocol,plies:moves.length,moves,positions,newGameReply:fresh,stopReply:stopped,exitCode:0,transcript:lines});
...
console.log('Gameplay: 2 persistent process sessions and 48 plies passed, including new game and stop');
